...
else if ( target.head != target.x || target.distance != 8.44259 ) {
    setAhead( target.y );
}
...
if ( target.head > 3.93307 && target.bearing != -0.978676 ) {
    ...
    if ( target.speed > target.x && target.x > 6.79894 ) {
        ...
        setAhead( target.y );
        if ( target.head != target.head || target.bearing > 8.54866 ) {
